...
function TaskForm() {
  ...
  useEffect(() => {
    if (id) {
      const fetchTask = async () => {
        try {
          const response = await axios.get(`https://task-manager-woad-three.vercel.app/tasks/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axios.put(`https://task-manager-woad-three.vercel.app/tasks/${id}`, task);
      } else {
        await axios.post('https://task-manager-woad-three.vercel.app/tasks', task);
      }
      navigate('/');
    } catch (error) {
